...
class PlateInference:
    ...
    def reset_tracker(self) -> None:
        try:
            self.plate_model.predictor = None
            logger.info("Plate YOLO tracker reset (predictor cleared).")
        except Exception as exc:
            logger.warning("Could not reset tracker: %s", exc)

    # Phát hiện biển số + Tracking
    ...
    def detect_plate(
        ...
        try:
            ...
            # Frame đầu ByteTrack chưa kịp gán ID → bỏ qua
            if boxes.id is None:
                return None, None, None
            ...
            track_id = int(boxes.id[best_idx].item())

            x1, y1, x2, y2 = (int(xyxy[0]), int(xyxy[1]),
                               int(xyxy[2]), int(xyxy[3]))

            # Giới hạn và mở rộng bbox thêm 5%
            h, w   = frame.shape[:2]
            ...
            if x2 <= x1 or y2 <= y1:
                logger.debug("Invalid bbox after clamping: [%d,%d,%d,%d]",
                             x1, y1, x2, y2)
                return None, None, None

            plate_crop = frame[y1:y2, x1:x2].copy()
            logger.debug("Plate detected [%d,%d,%d,%d] track_id=%d",
                         x1, y1, x2, y2, track_id)
            return plate_crop, [x1, y1, x2, y2], track_id
    ...
    def ocr_plate(self, plate_crop: np.ndarray, is_two_lines: bool) -> list[str]:
        ...
        try:
            ...
            if not is_two_lines:
                ...
                logger.debug("OCR result 2-lines: top='%s', bottom='%s'",
                             str_top, str_bottom)
    ...
    def process_frame(self, frame: np.ndarray) -> dict | None:
        plate_crop, bbox, track_id = self.detect_plate(frame)
        ...
        logger.debug(
            "plate raw=%s → formatted='%s' (two_lines=%s, track_id=%d)",
            raw_chars_list, formatted, is_two_lines, track_id,
        )
